...
export default async function synchronizeShippingOptions() {
    ...
    referenceStrings.forEach(async (ref: string) => {
        ...
        if (!event) return;
        const dTagId = ShippingOptionUtils.getShippingOptionId(event!.rawEvent() as unknown as ShippingOption);
        await shippingOptionsDb.put(`nostr-shipping-option-event:${dTagId}`, event.rawEvent());
    })
