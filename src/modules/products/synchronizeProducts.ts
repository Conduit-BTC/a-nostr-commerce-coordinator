...
export default async function synchronizeProducts() {
  ...
  console.log(
    '[synchronizeProducts]: Clearing out the product events database...'
  )
  ...
  console.log(
    '[synchronizeProducts]: Clearing out the shipping option events database...'
  )
  ...
  homeRelaySubscription.on('event', async (event: NDKEvent) => {
    ...
    console.log(
      `[synchronizeProducts]: Received Product event from HomeRelay: ${event.id}`
    )
    ...
    if (!dTag || !dTag[1]) {
      console.error(
        `[synchronizeProducts]: Product event ${product.id} missing d tag`
      )
      return
    }
    ...
    console.log(
      '[synchronizeProducts]: Storing product event in local database...'
    )
    await productsDb.put(`nostr-product-event:${productId}`, product)
    console.log(
      '[synchronizeProducts]: Product event stored in local database: ' +
    ...
    console.log(
      '[synchronizeProducts]: Broadcasting product event to relay pool...'
    )
    relayPool.relays.forEach((relay) => {
      console.log(
        `[synchronizeProducts]: Broadcasting product event to relay: ${relay.url}`
      )
      relay.publish(event)
    })
    console.log(
      '[synchronizeProducts]: Product event broadcasted to relay pool'
    )
  })
  ...
  await new Promise<void>((resolve) => {
    homeRelaySubscription.on('eose', () => {
      console.log(
        '[synchronizeProducts]: All events from HomeRelay received. Finishing up...'
      )
