...
export async function dmQueueEventHandler(queueItem: QueueItem<NostrEvent>) {
  ...
  try {
    ...
    if (order.success) {
      ...
      QueueRegistry.get(QUEUE_NAME.ORDERS)?.push({
        order: order.data,
        customerPubkey: sealJson.pubkey
      })
      QueueRegistry.get(QUEUE_NAME.DIRECT_MESSAGES)?.confirmProcessed(
        queueItem.id
      )
      // Handled by orderEventHandler
      return
    }
    ...
    getDb()
      .openDB({ name: DB_NAME.IGNORED_EVENTS })
      .put(`nostr-ignored-event:${event.id}`, true)
  } catch (error) {
    console.error(
      `[subscribeDirectMessages]: Failed to process order event: ${error}`
    )
    getQueue(QUEUE_NAME.DIRECT_MESSAGES).confirmProcessed(queueItem.id)
    return
  }
...
function handleRejectedOrder(event: NDKEvent, customerPubkey: string) {
  ...
  if (rejectedOrder.kind !== 16 || OrderUtils.getOrderId(rejectedOrder) == null)
    return
  ...
  const messageObj = {
    ...
    message:
      "It looks like you're using an e-commerce client doesn't create Order Events compatible with the GammaMarkets Market Spec. This Merchant can't settle this order, at this time. Please contact the dev team that maintains your client, and give them this link: https://github.com/GammaMarkets/market-spec/blob/main/spec.md"
  }
