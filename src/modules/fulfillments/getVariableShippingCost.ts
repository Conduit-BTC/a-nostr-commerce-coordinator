 ...
 */
export async function getVariableShippingCost(
  zip: string,
  ...
  try {
    ...
    for (const pkg of packages) {
      const payload: ShippingCostQuotePayload = createShippingCostQuotePayload(
        zip,
        pkg
      )
...
function getMerchantPackageSpecs(): MerchantPackageSpec[] {
  console.error(
    '[getVariableShippingCost > getMerchatPackagePreferences]: <<< NOT IMPLEMENTED. USING FAKE DATA. Fetch from MerchantSettings database, instead.'
  )
  return [
    {
      height: 12,
...
function createShippingCostQuotePayload(
  destinationZIPCode: string,
  pkg: Package
) {
  const originZIPCode = getValueFromDb(
    DB_NAME.SETTINGS,
    MERCHANT_SETTING.ZIP_CODE
  )
  if (!originZIPCode)
    throw new Error(
      "[getVariableShippingCost > createShippingCostQuotePayload]: Merchant Zip Code isn't set. Cannot calculate shipping estinate."
    )
 ...
 */
async function fetchShippingCostQuote(
  payload: ShippingCostQuotePayload
): Promise<{
  success: boolean
  cost?: number
  error?: { message: string; status: number }
  ...
  while (currentIndex < flattenedItems.length) {
    ...
    for (const spec of packageSpecs) {
      ...
      for (let i = currentIndex; i < flattenedItems.length; i++) {
        ...
        const itemVolume =
          (item.length || 1) * (item.width || 1) * (item.height || 1)
    ...
    if (!packed) {
      throw new Error(
        `Item at index ${currentIndex} does not fit in any provided packageSpec.`
      )
    }
  }
