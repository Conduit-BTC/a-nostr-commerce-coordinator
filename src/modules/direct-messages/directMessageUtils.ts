...
import type {
  ...
  ORDER_STATUS,
  FULFILLMENT_STATUS,
  NIP17_KIND as TYPE_NIP17_KIND,
  ORDER_MESSAGE_TYPE as TYPE_ORDER_MESSAGE_TYPE
} from '@/types/types'
import {
  NDKEvent,
  NDKPrivateKeySigner,
...
export async function sendDirectMessage(
  ...
  try {
    ...
    return {
      success: false,
      message:
        "Direct message was created, but didn't publish to the relay pool."
    }
  } catch (error) {
    console.error(error)
    return {
      success: false,
      message: 'There was an error while trying to send a direct message.'
    }
  }
}
...
export async function sendOrderStatusUpdateMessage({
  ...
  message,
  tags: extraTags
}: OrderStatusUpdateArgs): Promise<{ success: boolean; message: string }> {
  try {
    ...
    if (success)
      return { success: true, message: 'Order status DM sent successfully' }
    return {
      success: false,
      message:
        "Order status DM was created, but didn't publish to the relay pool."
    }
  } catch (error) {
    console.error(error)
    return {
      success: false,
      message: 'There was an error while trying to send an order status DM.'
    }
  }
}
...
export async function sendPaymentRequestMessage({
  ...
  amount,
  lnInvoice
}: PaymentRequestArgs): Promise<{ success: boolean; message: string }> {
  try {
    const kind = NIP17_KIND.ORDER_PROCESSING
    ...
    return {
      success: false,
      message:
        "Payment request was created, but didn't publish to the relay pool."
    }
  } catch (error) {
    console.error(error)
    return {
      success: false,
      message: 'There was an error while trying to send a payment request.'
    }
  }
}
...
export async function sendReceiptMessage({
  ...
  lnInvoice,
  lnPaymentHash
}: ReceiptArgs): Promise<{ success: boolean; message: string }> {
  try {
    ...
    return {
      success: false,
      message:
        "Payment confirmation was created, but didn't publish to the relay pool."
    }
  } catch (error) {
    console.error(error)
    return {
      success: false,
      message: 'There was an error while trying to send a payment confirmation.'
    }
  }
}

async function createNip17GiftWrapEvent(
  kind: TYPE_NIP17_KIND,
  recipient: string,
  message: string,
  type?: TYPE_ORDER_MESSAGE_TYPE,
  tags?: string[][]
