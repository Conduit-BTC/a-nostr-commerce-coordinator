...
export default async function subscribeDirectMessages() {
  console.log(
    `[subscribeDirectMessages]: Listening to Relay Pool for NIP-17 DMs addressed to ${process.env.PUBKEY}...`
  )
  ...
  subscription.on('event', async (event: NDKEvent) => {
    console.log(
      `[subscribeDirectMessages]: Received NIP-17 encrypted message: ${event.id}`
    )
    ...
    console.log(
      `[subscribeDirectMessages]: Checking if event is explicitly ignored...`
    )
    if (shouldIgnoreEvent(event.id)) return

    console.log(
      `[subscribeDirectMessages]: Adding direct message to queue: ${event.id}`
    )
