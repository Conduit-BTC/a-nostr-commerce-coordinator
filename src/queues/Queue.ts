...
export class Queue<T> {
    ...
    public confirmProcessed(itemId: string): void {
        if (!this.inFlightItems.delete(itemId)) {
            console.warn(`Attempted to confirm item ${itemId} but it was not found in in-flight items`)
        }
    }
    ...
    public requeueItem(itemId: string): void {
        ...
        if (item) {
            ...
            console.warn(`Attempted to requeue item ${itemId} but it was not found in in-flight items`)
        }
    }
